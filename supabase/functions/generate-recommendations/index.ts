...
async function generateCaptainRecommendations(gameweek: number, players: any[], fixtures: any[], teams: any[]) {
  try {
...
async function generateTransferRecommendations(gameweek: number, players: any[], fixtures: any[], teams: any[]) {
  try {
    // Prepare data for the AI
...
async function generateDifferentialRecommendations(gameweek: number, players: any[], fixtures: any[], teams: any[]) {
  try {
...
serve(async (req) => {
  try {
    ...
    if (eventsError) {
      return new Response(
        JSON.stringify({ error: `Failed to fetch current gameweek: ${eventsError.message}` }),
    ...
    if (playersError || !players) {
      return new Response(
        JSON.stringify({ error: `Failed to fetch players data: ${playersError?.message || 'No data'}` }),
        { status: 500, headers: { 'Content-Type': 'application/json' } }
      );
    }
    ...
    if (fixturesError || !fixtures) {
      return new Response(
        JSON.stringify({ error: `Failed to fetch fixtures data: ${fixturesError?.message || 'No data'}` }),
        { status: 500, headers: { 'Content-Type': 'application/json' } }
      );
    }
    ...
    if (teamsError || !teams) {
      return new Response(
        JSON.stringify({ error: `Failed to fetch teams data: ${teamsError?.message || 'No data'}` }),
        { status: 500, headers: { 'Content-Type': 'application/json' } }
      );
    }
    ...
    if (type === 'all' || type === 'captains') {
      const captainRecommendations = await generateCaptainRecommendations(currentGameweek, players, fixtures, teams);
    ...
    if (type === 'all' || type === 'differentials') {
      const differentialRecommendations = await generateDifferentialRecommendations(currentGameweek, players, fixtures, teams);

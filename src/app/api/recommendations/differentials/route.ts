...
export async function GET() {
  try {
    ...
    // Get differential recommendations without player join
    ...
    if (error) {
      throw new Error(`Failed to fetch differential recommendations: ${error.message}`);
    }
    ...
    console.error('Error fetching differential recommendations:', error);
    return NextResponse.json({ 
      error: error.message 
    }, { status: 500 });
  }
